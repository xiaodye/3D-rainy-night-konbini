...
环境音**全部由 Web Audio 实时合成，没有任何音频文件**。首次点击页面任意处便会自动响起
（浏览器要求一次用户手势），右上角 ♪ 图标可随时开关 —— 开启时呈暖黄色。

- **雨滴**：每秒 15–40 颗离散雨滴，每颗是 5–15ms 的窄带噪声脉冲（随机音高 1.5–4.3 kHz），
  随机立体声位置 —— 这才是"雨滴落在水面"，而不是一阵风声
- **雨幕底噪**：低通白噪声垫在雨滴之下，低通角频率随 20 秒周期的 LFO 漂移，模拟远处雨势
- **暖垫音**：A 小调持续和弦（A2 E3 A3 C4），轻微失谐 + 20 秒呼吸起伏 —— 像店里传出的旧音响
- **远雷**：每 20–60 秒随机一阵低频隆隆，可感知、不惊扰

面板里的「雨量」滑块实时联动雨滴密度与底噪强度。手动关闭后，本次会话内不再自动播放。
