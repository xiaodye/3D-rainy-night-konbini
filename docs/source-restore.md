...
**① `addLight()` 的颜色参数不生效 —— 已修复（含两处渲染调整）。**

`WetGround.addLight(pos, color, radius, intensity)` 内部调用 `THREE.Color.set(color)`，
而 three 的 `Color.set()` 只接受 `Color | number | string` —— 场景里 11 处调用传的都是
**`[r, g, b]` 数组**，会被静默忽略，`uLightCol` 始终保持初始的黑色（`uLightCount` 是 8，
颜色却全是 `[0,0,0]`），所以面板上的「地面灯光」滑块实际无效。

修在**一处**而不是十一处：`addLight` 里判断类型，数组走 `setRGB()`。调用方一行都不用改。

**但只修这个不够 —— 第一次修完直接过曝。** shader 里原本是逐像素线性累加：

```glsl
col += uLightCol[i] * att * uLightRad[i].y * uPoolStrength * mix(0.55, 1.0, wet);
```

单个灯在最近处就贡献 `1.0 × 1.0 × uPoolStrength`，而场景里给了 8 盏灯、半径 3.4–9.0 米，
店门口有 5–6 盏重叠 —— 叠加值轻松超过 4，整片湿沥青被冲成米白色，雨点涟漪与镜面反射全部消失。
把 `uPoolStrength` 从 1.2 调到 0.15 仍然如此：**它是线性增益，一动就越过白点，纯调数值治不了本。**

因此改了两处渲染逻辑（都在同一个循环里）：

1. **平方衰减** —— `att *= att`。灯正下方保持满强度，出了半径迅速衰减，
   让 8 个池子读作"几片光斑"而不是"一整层均匀泛亮"（原本的半径是按"照亮一片地面"给的，
   叠加起来必然糊成一片）。
2. **软饱和** —— 先累加"加权颜色 + 总权重"，再对亮度做 `lum / (1 + lum)`。
   颜色取加权平均，避免重叠把冷暖混合放大成白；亮度曲线保证 `0→0、1→0.5、∞→1`，
   即**无论怎么拉滑块都不会过曝**，面板整段范围都可用。

配套数值：默认 `uPoolStrength = 0.06`（约为关闭状态的"轻微暖意"——店门口可见、街道保持深色），
面板范围 `0–0.5`、步长 `0.005`（便于在低端微调）。三处同步：`ground.ts` 的 uniform 初值、
`index.ts` 的 `num('pool', …)`、`adapter.ts` 的 `WATER_PARAMS`。

> 逐档截图验证：`0`（深色基准，反射与涟漪完整）、`0.06`（默认，接近基准 + 门口暖意）、
> `0.15`（明显光池，仍保留反射）、`0.5`（上限，明亮但不白）。
> 想快速试别的值可以直接用 URL：`?pool=0.1`。
